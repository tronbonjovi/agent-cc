...
vi.mock("../server/board/session-enricher", () => ({
  ...
  buildSessionSnapshot: vi.fn((enrichment: any) => ({
    ...
    totalToolCalls: enrichment.totalToolCalls ?? 0,
    retries: enrichment.retries ?? 0,
    cacheHitRate: enrichment.cacheHitRate ?? null,
    maxTokensStops: enrichment.maxTokensStops ?? 0,
    webRequests: enrichment.webRequests ?? 0,
    sidechainCount: enrichment.sidechainCount ?? 0,
    turnCount: enrichment.turnCount ?? 0,
  })),
  cacheSnapshot: vi.fn(),
