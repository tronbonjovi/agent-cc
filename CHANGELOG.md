...
### Added
- **Messages redesign — Phase 1 & 2: timeline foundation, sidebar, and 7 message bubbles (3/6 tasks)** — first three tasks of the `messages-redesign` milestone shipped on branch `feature/messages-redesign`. **task001 — message timeline endpoint:** `GET /api/sessions/:id/messages` now returns a typed `TimelineMessage` stream covering all 7 variants (`user_text`, `assistant_text`, `thinking`, `tool_call`, `tool_result`, `system_event`, `skill_invocation`), with optional `?include=tree` enrichment attaching `treeNodeId` and `subagentContext` (agentId, agentType, description) to every message under a subagent root. Route-level `?types=` filter validates against a `TimelineMessageType` set; `totalMessages` reflects post-filter count. `meta.treeStatus: 'ok' | 'unavailable'` signals whether tree enrichment succeeded so clients can render a fallback banner. **task002 — session sidebar component:** `SessionSidebar.tsx` narrow left panel listing all sessions with project grouping, search, relative timestamps, and selection state — the entry point for the Messages tab. **task003 — 7 message rendering components (wave 1 + wave 2):** `bubbles/` directory containing `UserBubble` (markdown + hover timestamp), `AssistantBlock` (markdown, optional stop-reason pill, model-change badge), `ThinkingBlock` (collapsible, char-count label — `ThinkingMessage` has no token field), `ToolResultBlock` (success/error divergence, char+line-capped truncation), `SystemEventBlock` (inline muted annotation with subtype→icon picker), `ToolCallBlock` (compact header + expand for raw JSON, consumes tool renderer registry — no switch), and `SidechainGroup` (collapsible wrapper grouping messages by `subagentContext.agentId`, label `{agentType} — {description} (N messages)`, border stripe via shared `subagent-colors.ts` palette, fallback to generic "Sidechain (N)" when tree is unavailable). Central dispatcher `bubbles/dispatcher.ts` exports `renderMessage(msg, opts)` exhaustive over all 7 timeline variants with a `never` guard; `bubbles/index.ts` is a pure barrel that re-exports the dispatcher and every bubble. Split is deliberate — `SidechainGroup` imports `renderMessage` from `./dispatcher` to avoid a circular import back through the barrel. **Tool renderer registry** at `bubbles/tool-renderers/`: per-tool modules (`bash`, `read`, `grep`, `edit`, `write`, `agent`, `fallback`) plus `types.ts` and an `index.ts` exposing `TOOL_RENDERERS: Map<string, ToolRenderer>` and `getToolRenderer(name)` with guaranteed fallback. `Task` is aliased to `agentRenderer`. Each per-tool `Summary` component knows its own input fields and returns a one-line compact label (Bash → command, Read → file path + optional line range, Grep → pattern + scope, Edit → path + optional "replace all", Write → path + char count, Agent → `subagent_type: description`). Fallback renderer picks the first string-valued input key from a preferred list. 47 new structural tests for wave 2 following the wave 1 convention — `fs.readFileSync` + regex, no jsdom/RTL. Full suite: **5501 tests across 150 files**. Remaining tasks: task004 (ConversationViewer with scroll/keyboard nav), task005 (FilterBar + MessagesTab wiring), task006 (in-conversation search). (messages-redesign-task001, task002, task003 wave 1 + wave 2)
